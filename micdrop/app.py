...
from micdrop.utils.constants import (
    CATEGORICAL_COLS,
    NUMERIC_COLS,
    EXPECTED_COLUMNS,
    Y_VAR,
...
def predict():
    ...
    for col_nm in CATEGORICAL_COLS:
        df[col_nm] = df[col_nm].astype(
            CategoricalDtype(categories=latest_categorical_cols_dict[col_nm])
        )
    ...
    response.headers[
        "Content-Disposition"
    ] = f"attachment; filename={out_name}_predictions.csv"
    return response
